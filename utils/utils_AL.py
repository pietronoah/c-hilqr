...
def plotter2DBall(system, solver, h):
    ...
    axis[0, 0].plot(solver.x_trajectories[:, 0], solver.x_trajectories[:, 1],label="Solver solution")#, color='tomato')
    ...
    figure.suptitle('AL iterations: ' + str(solver.outer_loop_iter) + ', HiLQR iterations: ' + str(solver.total_iter_HiLQR) + ', J: ' + str(format_number(final_cost.numpy().item())), fontsize=12)
    ...
    delta_u = torch.zeros((solver.total_iter_HiLQR,))
    for i in range(solver.total_iter_HiLQR):
        temp = solver.u_trajectories_history[:,:,i] - solver.u_trajectories[:,:]
        delta_u[i] = torch.norm(temp)

    axis[2, 0].plot(np.linspace(1, solver.total_iter_HiLQR, num=solver.total_iter_HiLQR), delta_u, marker='*',label='Δu') 
    axis[2, 0].grid(True)
    axis[2, 0].set_xlabel('Iteration')
    axis[2, 0].set_ylabel("Δu")
    axis[2, 0].legend()

    axis[2, 1].plot(np.linspace(1, solver.total_iter_HiLQR, num=solver.total_iter_HiLQR), solver.J_history[:solver.total_iter_HiLQR], marker='*')#, "Blue")
    axis[2, 1].grid(True)
...
def animation2DBall(system, solver, h):
    ...
    ani = animation.FuncAnimation(fig=figure, func=update, frames=(h+1)*solver.total_iter_HiLQR, interval=20, blit=True)
...
def animation2DBall_low_lr_input(system, solver, h):
    ...
    #print(print(plt.style.available)) # List of availeable color theme
...
def print_logo():
    ...
    # Render the text
    text = custom_fig.renderText('AL-HiLQR Optimizer')
...
def print_header():
    header = ("iter", "objective", "ΔJ", "||e||", "Alpha", "LS", "CV")
...
column_widths = [6, 10, 10, 10, 10, 6, 6]

def print_current_iteration(iteration, objective, delta_J, error, alpha, ls, violation):
    current_row = (iteration, objective, delta_J, error, alpha, ls, str(int(violation*100))+"%")
    print(format_row(current_row))

def print_summary(AL_iter, HiLQR_iter, BP_iter, FP_iter, J, const_viol, time):
    print()
    print("Optimization Summary:")
    print("--------------------")
    print(f"Number of AL iterations: {AL_iter}")
    print(f"Number of HiLQR iterations: {HiLQR_iter}")
    print(f"Number of backward pass iterations: {BP_iter}")
    print(f"Number of forward pass iterations: {FP_iter}")
    print(f"Final cost (J): {J}")
    print(f"Constraint violation: {const_viol}")
    print(f"Time in the optimizer: {time} seconds")
    print()
